...
const removeSensitiveFields = (searchResults, ...fields) => {
  ...
  newSearchResults.map((searchResult) =>
    fields.map((fieldName) => delete searchResult[fieldName])
  );
  /* eslint-enable */
...
router.get(
  ...
  async (req, res) => {
    ...
    const [firstName, lastName] = await redisClient.hmget(
      userKey,
      'firstName',
      'lastName'
    );
...
router.get(
  ...
  [param('emailAddress').isEmail(), apiErrorReporter],
  async (req, res) => {
...
router.get('/users/recent', async (req, res) => {
  ...
  res
    .status(200)
    .json(removeSensitiveFields(searchResults, ...SENSITIVE_FIELD_NAMES));
});

// Get the top 100 users by number of checkins.
router.get('/users/bycheckins', async (req, res) => {
  const searchResults = await redis.performSearch(
    redis.getKeyName('usersidx'),
    '*',
    'SORTBY',
...
router.get(
  ...
  async (req, res) => {
    ...
    res
      .status(200)
      .json(
        removeSensitiveFields(searchResults, 'email', ...SENSITIVE_FIELD_NAMES)
      );
  }
);
